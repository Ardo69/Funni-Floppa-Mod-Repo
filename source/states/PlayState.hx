...
class PlayState extends states.MusicBeatState
{
	...
	public var shaderMap:Map<String, FlxRuntimeShader> = new Map<String, FlxRuntimeShader>();
	#end
	...
	public var hungerText:FlxText;

	public var hunger:Float = 1;
	...
	// private var moneyImage:AttachedSprite;
	public var moneyTxt:FlxText;
	...
	// shadowmario you stupid idiot these are used in songtxt
	// i forgor if we added that
	...
	// Lua shit
	public static var instance:states.PlayState;
	...
	override public function create()
	{
		...
		states.substates.GameOverSubstate.resetVariables();
		...
		switch (curStage)
		{
			...
			case 'backrooms':
				var midGround:BGSprite = new BGSprite('stages/backrooms', consistentPosition[0], consistentPosition[1]);
				midGround.setGraphicSize(Std.int(midGround.width * 1.4));
				midGround.updateHitbox();
				add(midGround);

			case 'ardocovers':
				var midGround:BGSprite = new BGSprite('stages/house-old', consistentPosition[0], consistentPosition[1]);
		...
		timeBarBG.yAdd = -4;
		// add(timeBarBG);
		timeBar = new FlxBar(timeBarBG.x + 4, timeBarBG.y + 4, LEFT_TO_RIGHT, Std.int(timeBarBG.width - 8), Std.int(timeBarBG.height - 30), this,
			'songPercent', 0, 1);
		...
		add(songTxt);
		songTxt.text = curSong + " (" + storyDifficultyText + ") " + "| Floppa Engine " + states.MainMenuState.modVersion;
	...
	public function initLuaShader(name:String, ?glslVersion:Int = 120)
	{
		...
		var foldersToCheck:Array<String> = [FileSystem.absolutePath("assets/shaders/")];
	...
	function startSong():Void
	{
		...
		FlxG.sound.playMusic(Paths.inst(states.PlayState.SONG.song), 1, false);
	...
	private function generateSong(dataPath:String):Void
	{
		...
		FlxG.sound.list.add(new FlxSound().loadEmbedded(Paths.inst(states.PlayState.SONG.song)));
		...
		for (section in noteData)
		{
			for (songNotes in section.sectionNotes)
			{
				...
				try
				{ // fix crashing on HL due to missing notetype
	...
	override public function update(elapsed:Float)
	{
		...
		if (controls.PAUSE && startedCountdown && canPause)
		{
			...
			if (ret != FunkinLua.Function_Stop)
			{
				...
				/*if (FlxG.random.bool(0.1))
				{
					...
					states.MusicBeatState.switchState(new GitarooPause());
				}
				else { */
				if (FlxG.sound.music != null)
				{
		...
		if (HungerSongs.contains(SONG.song.toLowerCase()))
		{
			hungerText.text = "Hunger: " + Highscore.floorDecimal(hunger * 100, 0) + "%";

			if (hunger <= 0.4)
				hungerText.color = FlxColor.YELLOW;
			else
				hungerText.color = FlxColor.WHITE;
		}
		...
		if (FlxG.keys.anyJustPressed(debugKeysCharacter) && !endingSong && !inCutscene)
		{
			...
			paused = true;
			cancelMusicFadeTween();
			states.MusicBeatState.switchState(new CharacterEditorState(SONG.player2));
		}
				...
				if (daNote.copyY)
				{
					...
					if (strumScroll && daNote.isSustainNote)
					{
						if (daNote.animation.curAnim.name.endsWith('end'))
						{
							...
							if (states.PlayState.isPixelStage)
							{
								daNote.y += 8 + (6 - daNote.originalHeightForCalcs) * states.PlayState.daPixelZoom;
							}
							else
							{
								daNote.y -= 19;
	...
	function openChartEditor()
	{
		switch (curSong.toLowerCase())
		{
			case 'players-fate' | 'floppasition' | 'flopparchy' | 'unknown-crapping' | 'monochrome':
				...
				paused = true;
				cancelMusicFadeTween();
				states.MusicBeatState.switchState(new states.JumpscareState());
				chartingMode = true;
	...
	function doDeathCheck(?skipHealthCheck:Bool = false)
	{
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead)
		{
			...
			if (ret != FunkinLua.Function_Stop)
			{
				...
				openSubState(new states.substates.GameOverSubstate(boyfriend.getScreenPosition().x - boyfriend.positionArray[0],
					boyfriend.getScreenPosition().y - boyfriend.positionArray[1], camFollowPos.x, camFollowPos.y));

				// states.MusicBeatState.switchState(new GameOverState(boyfriend.getScreenPosition().x, boyfriend.getScreenPosition().y));
	...
	function moveCameraSection(?id:Int = 0):Void
	{
		...
		{
			// camFollow.set(gf.getMidpoint().x + gf.camCeraPosition[0] + girlfriendCameraOffset[0], gf.getMidpoint().y + gf.cameraPosition[1] + girlfriendCameraOffset[1]);
			camFollow.set(gf.x + gf.cameraPosition[0] + girlfriendCameraOffset[0], gf.y + gf.cameraPosition[1] + girlfriendCameraOffset[1]);
			tweenCamIn();
		...
		if (ret != FunkinLua.Function_Stop && !transitioning)
		{
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					{
						states.MainMenuState.weekCompleted.set(WeekData.weeksList[storyWeek], true);
						...
						FlxG.save.data.weekCompleted = states.MainMenuState.weekCompleted;
						FlxG.save.flush();
					}
				...
				else
				{
					...
					trace(Paths.formatToSongPath(states.PlayState.storyPlaylist[0]) + difficulty);
					...
					states.PlayState.SONG = Song.loadFromJson(states.PlayState.storyPlaylist[0] + difficulty, states.PlayState.storyPlaylist[0]);
					FlxG.sound.music.stop();
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		if (!practiceMode && !cpuControlled)
		{
			...
			/*if (ClientPrefs.scoreZoom)
			{
				...
				scoreTxt.angle = (scoreDancinLeft ? -15 : 15);
				scoreTxtTween = FlxTween.tween(scoreTxt, {"scale.x": 1, "scale.y": 1, angle: 0}, 0.2, {
					onComplete: function(twn:FlxTween)
					{
						scoreTxtTween = null;
					}
				...
				if (sortedNotesList.length > 0)
				{
					for (epicNote in sortedNotesList)
					{
						for (doubleNote in pressNotes)
						{
							...
							{
								if (modchartObjects.exists('note${doubleNote.ID}'))
	...
	private function keyShit():Void
	{
		...
		if (controls.BUY_FOOD) // coolness and nice
		{
			var prevHunger = hunger;
			hunger = Math.min(1, hunger + ((money / 100) * 2));
			money -= Math.ceil(((hunger - prevHunger) / 2) * 100);
		}
	...
	override function beatHit()
	{
		...
		if (generatedMusic && states.PlayState.SONG.notes[Std.int(curStep / 16)] != null && !endingSong && !isCameraOnForcedPos)
		{
